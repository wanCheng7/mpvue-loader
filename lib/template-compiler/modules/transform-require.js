...
function rewrite (attrsMap, name, fileOptions) {
  ...
  if (value) {
    ...
    if (firstChar === '.') {
      ...
      // 重写路径，为了避免重名，在webpack输出目录下新建copy-assets目录，资源保存到这里
      // var assetOutputPath = path.join('copy-assets', path.relative(process.cwd(), assetPath).replace(/^src/, ''))
      var assetOutputPath = path.relative(process.cwd(), assetPath).replace(/^src/, '').slice(1);
